    ...
    for i in range(length):#对于每条数据
        for num in QID:#对于每个属性
            if num==QID[5]:#为第五个疾病敏感属性，则根据级别匿名
                ...
                '''
                #PayOff后面又算了一次，向上翻了一层
                if(n==0):
                    print(PayOff,data[i][num],Trees[num].depth())
                    n=n+1
                '''
    ...
    while tmp_data: #执行到原始数据库中没有数据为止
        ...
        for i in range(len(tmp_data)):
            ...
            if Equal(EQ[:-1], each, QID):
                Anonymity_data.append(each) #添加相同数据
                #del tmp_data[i]
                ls.append(i)    #mark下相同记录的位置，以便待会删除
                EQ[-1] = EQ[-1] + 1  # 更新K值
        tmp_data=Remove2(tmp_data,ls)   #删除掉已经添加完的相同数据
