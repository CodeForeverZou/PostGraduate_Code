...
data=[] #数据元组
attributes = ('age','weight','marital-status','race','sex','native-country')  #数据的所有属性值
QID = (0,1,2,3,4,5) #要进行匿名的属性值
QID_Th=(0.4 ,0.4 ,0,0 ,0,0)#阈值：每个属性的信息损失量能承受的最大值
Anonymity_data=[]
...
def Anonymity():
    ...
    '''
    #====================================
    #此时的Anonymity_data还有些只出现一次的，
    tmp_data=Anonymity_data[:]
    tmp_data2=[]
    ls=[]
    for i in tmp_data:
        if i[-1]==1:
