...
def loadDataSet():
    ...
    L=[]
    fo=open("D:\code\python\MASK/data5\\3_2.data",'r')
    str=fo.readline()
    while(str!=''):
        li=re.findall('\d+',str)
        L.append(li[1:])
...
if __name__=='__main__':
    ...
    fo=open("D:\code\python\MASK/data5\\pic_result_frequent2.pkl","wb")
    pickle.dump(L,fo)
    fo.close()
    
    
    fo=open("D:\code\python\MASK/data5\\result_supp2.txt","w")
    for i in suppData:
        fo.write(str(i))
        fo.write('\t')
    ...
    fo=open("D:\code\python\MASK/data5\\result_frequent2.txt","w")
    for i in L:
        fo.write(str(i))
        fo.write('\n')
        fo.write('\n')
