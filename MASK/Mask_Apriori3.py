...
def loadDataSet():
    ...
    L=[]
    fo=open("D:\code\python\MASK\\3_3.data",'r')
    str=fo.readline()
    while(str!=''):
        li=re.findall('\d+',str)
        L.append(li[1:])
...
def scanD(dataSet, Ck, minSupport):
            ...
            for items_dic in ssCnt[can]:
                ...
                for item in items_dic:
                    if item not in tid:
                        flag=0
                        break
                    if item==items_dic[-1] and item not in tid:
                        n=n+1
    ...
    #print(ssCnt)
    f1=open("D:\code\python\MASK\\1.txt","w")
    ...
    for key in ssCnt:
        ...
        support=0
        for j in range(pow(2,len(Ck[0])-1)):
            #print(j)
            #supp1=ssCnt[key]
            if j==0:    #没有出现的情况，即值为0
                supp1=numItems-sum(v_result)
                support=support+supp1*M_[0][0]
            else:       #对应编号M_[0][k_result]，个数
                #supp1=ssCnt[key]
                supp1=v_result[j-1]
                support=support+supp1*M_[0][k_result[j-1]]
